...
class ResNetDesigner3D(nn.Module):
    def __init__(self, input_size=[1, 169, 208, 179]):
        ...
        self.conv1 = nn.Conv3d(input_size[0], 2 ** (3)  , kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm3d(2 ** (3))
        self.act1 = nn.ELU(inplace=True)
        self.maxpool = PadMaxPool3d(kernel_size=3, stride=2)
