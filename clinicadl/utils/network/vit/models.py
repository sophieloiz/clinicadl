...
class ViTVNet(ViT):
    def __init__(self, *, image_size=(169, 208, 179), patch_size=13, num_classes=2, dim=1024, depth=6, heads=16, mlp_dim=2048, pool='cls', channels=1, dim_head=64, dropout=0., emb_dropout=0.,gpu=True):
        ...
        num_patches = (image_size[0] // patch_size) * \
            (image_size[1] // patch_size)*(image_size[2]-3 // patch_size)
        print(num_patches)
        ...
        super().__init__(
            patch_to_embedding=patch_to_embedding, transformer=transformer, to_latent=to_latent, mlp_head=mlp_head, dropout=dropout, pos_embedding=pos_embedding, cls_token= cls_token, patch_size=patch_size,gpu=gpu
        )
